...
export const UserMenu = ({ user, profile, isSuperAdmin, isAdmin, onLogout }: UserMenuProps) => {
  ...
  const handleLogout = async () => {
    try {
      ...
      toast({ 
        ...
        description: "À bientôt sur MontDePiété Pro!",
        duration: 3000
      });
      navigate("/");
  ...
  const getRoleBadge = () => {
    ...
    return { label: "Agent", color: "bg-gradient-to-r from-gray-600 to-gray-700", icon: User };
  };
  ...
  const menuItems = [
    ...
    { icon: Building, label: "Mon Agence", action: () => navigate("/agency"), description: "Informations de l'agence" },
  ...
  if (isAdmin || isSuperAdmin) {
    menuItems.splice(1, 0,
      { icon: UserCog, label: "Équipe", action: () => navigate("/team"), description: "Gérer l'équipe" },
      { icon: UserCog, label: "Ajouter un agent", action: () => navigate("/team/add"), description: "Inviter un nouveau membre" }
    );
  }
  ...
  if (isSuperAdmin) {
    menuItems.splice(2, 0, { 
      ...
      action: () => navigate('/settings/platform'), 
          ...
          >
            <AlertDialogHeader className="text-center pb-4">
              ...
              <AlertDialogDescription className="text-gray-600">
                Êtes-vous sûr de vouloir vous déconnecter de votre compte MontDePiété Pro ?
              </AlertDialogDescription>
            </AlertDialogHeader>
